...
type EditConsignmentButtonProps = {
  consignmentId: string;
};

export function EditConsignmentButton({
  consignmentId,
}: EditConsignmentButtonProps) {
  ...
  const handleEdit = () => {
    const adminPassword = window.prompt(
      "Enter admin password to edit this consignment."
    );
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setError(null);
        router.push(
          `/consignments/${consignmentId}/edit?pw=${encodeURIComponent(adminPassword)}`
        );
      } else {
