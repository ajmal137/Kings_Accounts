...
export async function updateInvoice(rawData: unknown) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      // 1. Delete old ledger entries linked to this invoice
      await tx.ledgerEntry.deleteMany({
        where: { linkedInvoiceId: id },
      });
